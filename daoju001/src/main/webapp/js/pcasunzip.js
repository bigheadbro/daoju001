...
$(function(){
	...
	var address;
	loaddata();
	$("body").on("click",".closeaddr",function(){
		addrshow=false;
		cleardata();
		p="";
		c="";
	...
	$("body").on("click",".item-del li",function(){
		p="";
		c="";
		a="";
		$(".item-del li").addClass("nostyle");
		$(".item-del li a.tabClose").remove();
		$(".item-check [id^=prov]").filter(".current").removeClass("current");
		$("#pca").empty();
		$("#city").empty().removeClass("loc-pad");
		$("#area").empty().removeClass("loc-pad");
	});
	$("body").on("click",".item-check [id^=prov]",function(){
		if(p!=""){
			$("#city").empty().removeClass("loc-pad");
			$("#area").empty().removeClass("loc-pad");
			$("#pca").empty();
		...
		p="";
		$("#city").empty().addClass("loc-pad");
		$(".item-check [id^=prov]").filter(".current").removeClass("current");
		$(this).addClass("current");
		num = parseAddr($(this).attr("id"));
		cities=provinceArray[num].getCity();
		for(i=0;i<cities.length;i++){
			$("#city").append(checkitem+"<a id=\"city"+i+"\" title=\""+cities[i].getName()+"\">"+cities[i].getName()+"</a>"+endspan);
		}
		$("#showprov").empty();
		$("#showcity").empty();
		$("#showarea").empty();
		p=provinceArray[num].getName();
	});
	$("body").on("click",".item-check [id^=city]",function(){
		if(p!=""){
			$("#area").empty().removeClass("loc-pad");
			$("#pca").empty();
			$(".item-del li").addClass("nostyle");
		...
		c="";
		$("#area").empty().addClass("loc-pad");
		$(".item-check [id^=city]").filter(".current").removeClass("current");
		$(this).addClass("current");
		num = parseAddr($(this).attr("id"));
		areas=cities[num].getArea();
		for(i=0;i<areas.length;i++){
			$("#area").append(checkitem+"<a id=\"area"+i+"\" title=\""+areas[i].getName()+"\">"+areas[i].getName()+"</a>"+endspan);
		}
		$("#showcity").empty();
		$("#showarea").empty();
		c=cities[num].getName();
	});
	$("body").on("click",".item-check [id^=area]",function(){
		if(a!=""){
			$(".item-del li a.tabClose").remove();
		}
		a="";
		$(".item-check [id^=area]").filter(".current").removeClass("current");
		$(this).addClass("current");
		num = parseAddr($(this).attr("id"));
		$("#showarea").empty();
		a=areas[num].getName();
		$("#pca").text(p+"-"+c+"-"+a);
		$("#pca").parent("li").removeClass("nostyle");
		$("#pca").after("<a class=\"tabClose\">×</a>");
	});
});
function cleardata(){
	$(".item-check [id^=prov]").filter(".current").removeClass("current");
...
function loaddata(){
	for(i=0;i<provinceAll.length;i++){
		citywitharea=provinceAll[i].split('$')[1].split('|');
		var cityArray = new Array();
		for(j=0;j<citywitharea.length;j++){
			areaAll=citywitharea[j].split(',');
			var areaArray = new Array();
			//traverse city,area,area
			for(k=0;k<areaAll.length;k++){
				if(k>0){
					var areaInst = new area(areaAll[k],null);
					areaArray.push(areaInst);
				}
			}
			var cityInst = new city(areaAll[0],null,areaArray);
			for(m=0;m<areaArray.length;m++){
				areaArray[m].setCity(cityInst);
			}
			cityArray.push(cityInst);
		}
		var prov = new province(provinceAll[i].split('$')[0],cityArray);
		for(m=0;m<cityArray.length;m++){
			cityArray[m].setProvince(prov);
		}
		provinceArray.push(prov);
  }
}
function province(name,city)
{
	this.name=name;
	this.city=city;
...
function city(name,province,area)
{
	this.name=name;
	this.area=area;
